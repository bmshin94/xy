...
## 프로젝트 개요
복잡한 데이터 통계를 단 몇 줄로 아름답고 세련된 인터랙티브 차트로 그려내는 "초고속 파이썬 데이터 차트 공방"
밋밋했던 숫자 표를 마우스 반응형 그래프, 히트맵, 시계열 차트로 탈바꿈시켜 데이터의 숨은 인사이트를 직관적으로 전달
데이터 분석가와 비즈니스 기획자가 고객과 경영진의 시선을 사로잡는 명품 대시보드를 완성하도록 돕는 시각화 무기
